...
-- WALK SPEED 30 — добавлено после Anti Fall в табе Player
mkTog(t2, "WalkSpeed 30", false, function(v)
    if v then
        if not _G.FloppaHub_WalkSpeedData then
            ...
            if plr.Character then
                setSpeed(plr.Character)
            end

            data.charConn = plr.CharacterAdded:Connect(setSpeed)
            data.heartbeatConn = game:GetService("RunService").Heartbeat:Connect(function()
                local char = plr.Character
                if not char then return end
        ...
        if _G.FloppaHub_WalkSpeedData then
            if _G.FloppaHub_WalkSpeedData.heartbeatConn then
                _G.FloppaHub_WalkSpeedData.heartbeatConn:Disconnect()
            end
...
-- RAKE INSTA KILL (переименовано с Rake Kill)
