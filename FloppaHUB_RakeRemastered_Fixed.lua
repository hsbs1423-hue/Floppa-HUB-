...
-- ФИКС: CanvasGroup заменён на Frame (убирает размытие текста при ресайзе)
...
local function applyESP(o, col, txt, key)
    ...
    if not cc then
        ...
        l.Parent = b
        -- ТОНКИЕ ЧЁРНЫЕ ОБВОДКИ К ESP НАДПИСЯМ
...
-- WALK SPEED 30 — рабочий подход через GetPropertyChangedSignal
mkTog(t2, "WalkSpeed 30", false, function(v)
    if v then
        if not _G.FloppaHub_WalkSpeedData then
            _G.FloppaHub_WalkSpeedData = {}
            local data = _G.FloppaHub_WalkSpeedData
            local TARGET_SPEED = 30

            local function setCharacterSpeed(char)
                if not char then return end
                local hum = char:WaitForChild("Humanoid", 5)
                if hum then
                    hum.WalkSpeed = TARGET_SPEED
                    data.wsConn = hum:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
            ...
            if plr.Character then
                setCharacterSpeed(plr.Character)
            end

            data.charConn = plr.CharacterAdded:Connect(function(newChar)
                task.wait(0.5)
                pcall(setCharacterSpeed, newChar)
            end)
        end
        _G.FloppaHub_WalkSpeedData.active = true
    else
        if _G.FloppaHub_WalkSpeedData then
            if _G.FloppaHub_WalkSpeedData.wsConn then
                _G.FloppaHub_WalkSpeedData.wsConn:Disconnect()
            end
            if _G.FloppaHub_WalkSpeedData.charConn then
                _G.FloppaHub_WalkSpeedData.charConn:Disconnect()
            end
            local char = plr.Character
            if char then
            ...
            _G.FloppaHub_WalkSpeedData = nil
        end
    end
end)
...
-- RAKE INSTA KILL (переименовано)
mkTog(t3, "Rake Insta Kill", false, function(v)
    if v then
